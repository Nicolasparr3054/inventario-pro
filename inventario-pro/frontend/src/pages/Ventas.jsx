      ...
      <div className="table-wrapper" style={{marginBottom:20}}>
        <div className="table-header">
          ...
          <div className="flex gap-8" style={{flexWrap:'wrap'}}>
            <input className="input" type="date" value={filters.desde} onChange={e=>setFilters(p=>({...p,desde:e.target.value}))} style={{width:140}}/>
            <input className="input" type="date" value={filters.hasta} onChange={e=>setFilters(p=>({...p,hasta:e.target.value}))} style={{width:140}}/>
            <select className="input" value={filters.estado} onChange={e=>setFilters(p=>({...p,estado:e.target.value}))} style={{width:130}}>
        ...
        {loading ? <div className="spinner"/> : (
          <table>
            <thead><tr><th>N° Venta</th><th>Cliente</th><th>Items</th><th>Subtotal</th><th>Descuento</th><th>Total</th><th>Método</th><th>Estado</th><th>Fecha</th><th></th></tr></thead>
            <tbody>
              {ventas.length ? ventas.map(v=>(
                ...
                <tr><td colSpan={10}><div className="empty-state">No hay ventas registradas</div></td></tr>
              )}
            </tbody>
          </table>
        )}
