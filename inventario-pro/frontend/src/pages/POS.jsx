// ══════════════════════════════════════════════════════════════
//  POS.jsx  ·  V5 – POS + escáner cámara + descuentos + caja
// ══════════════════════════════════════════════════════════════
...
export default function POS() {
  ...
  const [descuentoObj, setDescuentoObj]     = useState(null); // descuento aplicado
  ...
  const [camMode, setCamMode]       = useState(false);   // V5: escáner cámara
  const [turnoActivo, setTurnoActivo] = useState(null);  // V5: caja
  ...
  // V5: Verificar turno activo (solo cajeros)
  useEffect(() => {
  ...
  // Enfoque en input de escaneo
  useEffect(() => {
    if (scanMode && scanRef.current) scanRef.current.focus();
  }, [scanMode]);

  // V5: Escáner de cámara con html5-qrcode
  useEffect(() => {
    if (!camMode) {
    ...
    const startScanner = async () => {
      ...
      try {
        ...
        await scanner.start(
          ...
          async (decodedText) => {
            await buscarCodigo(decodedText);
          },
          () => {}
        );
      } catch (err) {
  ...
  // V5: Aplicar descuento por código
  const aplicarDescuento = async () => {
    ...
    try {
      ...
      if (data.tipo === 'porcentaje') {
        monto = (subtotal * parseFloat(data.valor)) / 100;
      } else {
        monto = parseFloat(data.valor);
      }
      setDescuentoObj(data);
      setDescuentoMonto(monto);
      toast.success(`Descuento "${data.nombre}" aplicado`);
    } catch (err) {
      toast.error(err.response?.data?.error || 'Código inválido');
      setDescuentoObj(null); setDescuentoMonto(0);
    }
  };

  const quitarDescuento = () => {
    setDescuentoObj(null); setDescuentoMonto(0); setCodigoDesc('');
  };
  ...
  const checkout = async () => {
    ...
    // V5: Verificar turno para cajeros
    if (user?.rol === 'cajero' && !turnoActivo) {
      return toast.error('Debes abrir un turno de caja antes de vender');
    }
    setLoading(true);
    try {
      const payload = {
        ...
        metodo_pago: metodo,
        descuento: Number(descuentoMonto),
        descuento_id: descuentoObj?.id || null,
        descuento_codigo: descuentoObj?.codigo || null,
        turno_id: turnoActivo?.id || null,
      };
      const { data } = await api.post('/ventas', payload);
      const { data: reciboData } = await api.get(`/ventas/${data.id}/recibo`);
      setRecibo(reciboData);
      setCart([]); setDescuentoMonto(0); setDescuentoObj(null);
      setCodigoDesc(''); setClienteId(''); setConfirm(false);
...
<body>
  <div class="center"><div class="title">📦 ${recibo.empresa || 'Inventario Pro'}</div><div>Recibo de Venta</div></div>
  ...
  <div class="row"><span>Cliente:</span><span>${recibo.cliente_nombre || 'General'}</span></div>
  <div class="row"><span>Método:</span><span>${recibo.metodo_pago}</span></div>
  <div class="divider"></div>
  ${recibo.detalles.map(d => `<div class="row"><span>${d.producto_nombre} x${d.cantidad}</span><span>$${Number(d.subtotal).toLocaleString('es-CO')}</span></div>`).join('')}
  <div class="divider"></div>
        ...
        <div className="pos-products">
          <div style={{ marginBottom:14, display:'flex', gap:8, flexWrap:'wrap' }}>
            ...
            <button
              ...
              onClick={() => { setScanMode(s => !s); setCamMode(false); }}
            >📟 Escáner</button>
            <button
              className={`btn btn-sm ${camMode ? 'btn-primary' : 'btn-ghost'}`}
              onClick={() => { setCamMode(s => !s); setScanMode(false); }}
            >📷 Cámara</button>
          </div>

          {/* Escáner teclado */}
          {scanMode && (
            <div style={{ display:'flex', gap:10, alignItems:'center', background:'var(--accent-bg)',
              border:'1px solid var(--accent)', borderRadius:8, padding:'10px 14px', marginBottom:12 }}>
              ...
              <input ref={scanRef} value={scanInput} onChange={e => setScanInput(e.target.value)}
                onKeyDown={handleScanKey} className="input" style={{ width:160, fontSize:12 }} placeholder="Código..." />
            </div>
          )}
          ...
          <div className="product-cards-grid">
            ...
            {!filtered.length && (
              <p style={{ color:'var(--text3)', gridColumn:'1/-1', padding:40, textAlign:'center' }}>Sin productos disponibles</p>
            )}
          </div>
        </div>
        ...
        <div className="cart-panel">
          ...
          <div className="cart-footer">
            <div style={{ marginBottom:12, display:'flex', flexDirection:'column', gap:8 }}>
              ...
              {/* Descuento manual (solo si no hay código aplicado) */}
              {!descuentoObj && (
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  <label style={{ fontSize:12, color:'var(--text2)', whiteSpace:'nowrap' }}>Desc. manual $</label>
                  <input className="input" type="number" min="0" value={descuentoMonto}
                    onChange={e => setDescuentoMonto(e.target.value)} style={{ fontSize:12 }}/>
                </div>
              )}
            </div>
            ...
            {Number(descuentoMonto) > 0 && (
              <div className="cart-total-row">
                <span style={{ color:'var(--green)' }}>
                  {descuentoObj ? `Descuento (${descuentoObj.tipo === 'porcentaje' ? descuentoObj.valor + '%' : fmt(descuentoObj.valor)})` : 'Descuento'}
                </span>
            ...
            {turnoActivo && (
              <div style={{ fontSize:11, color:'var(--green)', textAlign:'center', marginTop:6 }}>
                🟢 Turno #{turnoActivo.id} activo
              </div>
            )}

            <button
              className="btn btn-primary"
              disabled={!cart.length || loading || sinTurno}
          ...
          <div className="modal" style={{ maxWidth:360 }}>
            ...
            <div className="modal-body">
              ...
              <p style={{ fontSize:12, color:'var(--text3)', marginTop:8 }}>
                ...
                {descuentoObj ? ` · Descuento: ${descuentoObj.codigo}` : ''}
              </p>
            </div>
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth:440 }}>
            ...
            <div className="modal-footer" style={{ flexWrap:'wrap', gap:6 }}>
              ...
              <button className="btn btn-secondary" onClick={compartirWhatsApp}
