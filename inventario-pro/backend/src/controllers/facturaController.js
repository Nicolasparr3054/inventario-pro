// ══════════════════════════════════════════════════════════════
//  facturaController.js  ·  V6 – + Envío de factura por email
// ══════════════════════════════════════════════════════════════
...
exports.enviarFacturaPorEmail = async (req, res) => {
  try {
    ...
    const emailDestino = req.body.email || venta.cliente_email;
    if (!emailDestino) return res.status(400).json({ error: 'No hay email destinatario' });
...
<div class="invoice">
  ...
  <div class="footer">
    ...
    await transporter.sendMail({
      from: `"${empresa.nombre}" <${process.env.SMTP_USER}>`,
      to: emailDestino,
      subject: `Factura ${venta.numero_venta} - ${empresa.nombre}`,
      html: htmlGenerado,
    });

    res.json({ message: `Factura enviada a ${emailDestino}` });
  } catch (err) {
    console.error('Error enviando factura por email:', err);
    res.status(500).json({ error: err.message || 'Error enviando factura', details: err.code || err.response?.body || null });
  }
};